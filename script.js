...
function create_snow_lake() {
  ...
  var falling_time = Math.floor(Math.random() * 100) + 5; // Faster falling time between 5 and 35 seconds (faster)
  var flake_pos = Math.floor(Math.random() * 120); // Random position between 0% and 300% of the viewport width
  var flake_size = (Math.floor(Math.random() * 150) + 1) / 150; // Larger size range between 0.01 and 1.33
...
$(window).on("load", function () {
  // Set an interval to create and remove snowflakes every 50 milliseconds
  interval = setInterval(function () {
    create_snow_lake();
    remove_snow_lake();
  }, 50); // Increased the frequency of snowflakes (faster snowflake creation)
});


// Mobile Menu Toggle (for completeness)
